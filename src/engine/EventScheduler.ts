...
export class EventScheduler {
  ...
  tick(): void {
    ...
    // 6. Flash sale proximity trigger. Sales appear when the player is near a
    ...
    // 7. Leaderboard proximity check -> fire alerts when close.
    ...
    // 8. Trial perk expiry check -> remove expired perks.
    usePlayerStore.getState().expireTrialPerks();

    // 9. Map store stays subscribed; no per-tick map work needed here.

    // 10. Process scheduled event queue.
